                ...
                for (int j = 0; j < imageToRead.getHeight(); j++) {
                  ...
                  for (int x = (int) (-R); x <= (int) R; x++) {
                    for (int y = (int) (-R); y <= (int) R; y++) {
                      int ii = iii + x;
                      int jj = j + y;
                      if (Math.sqrt(x * x + y * y) <= R && ii >= 0 && ii < imageToRead.getWidth() && jj >= 0
                          && jj < imageToRead.getHeight()) {
                        PointCIEDE point1 = new PointCIEDE(ii, jj, imageToRead.getRGB(ii, jj));
                        if (point.compare(point1) <= dist) {
                  ...
                  Color newColor = new Color((int) pixR[iii][j], (int) pixG[iii][j], (int) pixB[iii][j],
                      origColor.getAlpha());
