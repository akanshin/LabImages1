...
public class PointCIEDE {
  ...
  public double compare(PointCIEDE point) {
    ...
    if (lab1[2] == 0 && a1Prime == 0)
       ...
       * addition of 2􏰏 to negative hue angles.
       */
      if (hPrime1 < 0)
        hPrime1 += deg360InRad;
    }
    ...
    /* Equation 15 */
    double T = 1.0 - (0.17 * Math.cos(barhPrime - degreeToRadian(30.0))) + (0.24 * Math.cos(2.0 * barhPrime))
    ...
    /* Equation 22 */
    double deltaE = Math.sqrt(Math.pow(deltaLPrime / (k_L * S_L), 2.0) + Math.pow(deltaCPrime / (k_C * S_C), 2.0)
        + Math.pow(deltaHPrime / (k_H * S_H), 2.0) + (R_T * (deltaCPrime / (k_C * S_C)) * (deltaHPrime / (k_H * S_H))));
  ...
  public double easycmp(PointCIEDE point) {
    ...
    double[] Lab2 = point.getLab();
    return Math
        .sqrt(Math.pow(Lab1[0] - Lab2[0], 2.0) + Math.pow(Lab1[1] - Lab2[1], 2.0) + Math.pow(Lab1[2] - Lab2[2], 2.0));
  }
}
